...
(function() {
  var argv, banner, beautify, coffee, concat, fs, gulp, header, pkg, sourcemaps, sources, uglify, util, watch, wrap;
  ...
  gulp.task('build', function() {
    ...
    uberbox = gulp.src(sources).pipe(coffee({
      bare: true
    }).on('error', util.log)).pipe(concat('uberbox.js')).pipe(wrap({
      src: 'exports.js.template'
    })).pipe(header(banner())).pipe(beautify()).pipe(gulp.dest('dist'));
