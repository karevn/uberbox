class Uberbox.Lightbox extends Uberbox.SlidingWindow
  ...
  ui:
    ...
    prev: '.uberbox-prev'
  events:
    'click @ui.next': (-> unless @ui.next.is('.uberbox-disabled')
      @currentItemView.model.next().activate() )
    'click @ui.prev': (-> unless @ui.prev.is('.uberbox-disabled')
      @currentItemView.model.prev().activate())
  ...
  scrollNext: (item)->
    ...
    if @currentItemView.model.isPrev(item)
      ...
      @currentItemView = @nextItemView
      if @currentItemView.model.next()
        @nextItemView = @createChildView(@currentItemView.model.next(),
        prev: @currentItemView)
      else
        @nextItemView = null
    else
  ...
  scrollPrev: (item)->
    ...
    if @currentItemView.model.isNext(item)
      ...
      @currentItemView = @prevItemView
      if @currentItemView.model.prev()
        @prevItemView = @createChildView(@currentItemView.model.prev(),
          next: @currentItemView)
      else
        @prevItemView = null
    else
      @prevItemView.remove() if @prevItemView
